#!/usr/bin/env bash
# Regression test for .gsd-recipe/scripts/install-recipe-run-phase.sh
# (TASK-024). Matches the convention of
# bench/tests/test-install-recipe-prd-intake.sh /
# bench/tests/test-install-recipe-planning-policy.sh — single-file staging
...
# Run: ./bench/tests/test-install-recipe-run-phase.sh
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
INSTALLER="$REPO_ROOT/.gsd-recipe/scripts/install-recipe-run-phase.sh"
...
# 2. Fresh install stages the skill at .cursor/skills/recipe-run-phase/SKILL.md
...
check "fresh install stages .cursor/skills/recipe-run-phase/SKILL.md" "$?"

LEDGER_COUNT1="$(python3 -c "import json; print(len(json.load(open('$TARGET1/.gsd-recipe/ledger.json'))['recipe-run-phase']))")"
[ "$LEDGER_COUNT1" = "1" ]
...
STAGED="$TARGET1/.cursor/skills/recipe-run-phase/SKILL.md"

grep -q "execute_started" "$STAGED" && rc=0 || rc=$?
check "staged skill references execute_started" "$rc"
grep -q "execute_complete" "$STAGED" && rc=0 || rc=$?
check "staged skill references execute_complete" "$rc"
grep -qi "Prerequisites" "$STAGED" && rc=0 || rc=$?
check "staged skill references the Prerequisites-table soft gate" "$rc"
grep -qi "soft gate" "$STAGED" && rc=0 || rc=$?
check "staged skill documents the gate as a soft (not hard) gate" "$rc"
grep -q "depends_on" "$STAGED" && rc=0 || rc=$?
check "staged skill references the non-blocking depends_on reminder" "$rc"
grep -qi "never fill" "$STAGED" && rc=0 || rc=$?
...
check "staged skill documents execute_wave handling as explicitly out of scope" "$rc"
...
"$INSTALLER" --yes --target "$TARGET1" >/dev/null
LEDGER_COUNT2="$(python3 -c "import json; print(len(json.load(open('$TARGET1/.gsd-recipe/ledger.json'))['recipe-run-phase']))")"
[ "$LEDGER_COUNT2" = "1" ]
...
[ ! -f "$TARGET2/.cursor/skills/recipe-run-phase/SKILL.md" ]
check "uninstall removes the staged skill" "$?"
LEDGER_AFTER="$(python3 -c "import json; d=json.load(open('$TARGET2/.gsd-recipe/ledger.json')); print('recipe-run-phase' in d)")"
[ "$LEDGER_AFTER" = "False" ]
check "uninstall clears the component's ledger entry" "$?"
[ ! -d "$TARGET2/.cursor/skills/recipe-run-phase" ]
...
cp -R "$REPO_ROOT" "$COPY"
(cd "$COPY" && ./.gsd-recipe/scripts/install-recipe-run-phase.sh --yes >/dev/null 2>&1) && rc=0 || rc=$?
check "self-install into a copy of this repo does not error (src==dest collision handled)" "$rc"
(cd "$COPY" && ./.gsd-recipe/scripts/install-recipe-run-phase.sh --uninstall >/dev/null 2>&1) && rc=0 || rc=$?
check "self-install uninstall does not error" "$rc"
[ -f "$COPY/.gsd-recipe/templates/recipe-run-phase-SKILL.md" ]
...
# pattern as bench/tests/test-install.sh's own TASK-012 composition checks.
INSTALL_SH="$REPO_ROOT/.gsd-recipe/scripts/install.sh"
grep -q "RECIPE_RUN_PHASE_INSTALLER" "$INSTALL_SH" && rc=0 || rc=$?
check "install.sh declares RECIPE_RUN_PHASE_INSTALLER" "$rc"
grep -q 'RECIPE_RUN_PHASE_INSTALLER" --yes' "$INSTALL_SH" && rc=0 || rc=$?
check "install.sh's install() invokes RECIPE_RUN_PHASE_INSTALLER --yes" "$rc"
grep -q 'RECIPE_RUN_PHASE_INSTALLER" --uninstall' "$INSTALL_SH" && rc=0 || rc=$?
check "install.sh's uninstall() cascades to RECIPE_RUN_PHASE_INSTALLER --uninstall" "$rc"
grep -q 'ledger_has_component "recipe-run-phase"' "$INSTALL_SH" && rc=0 || rc=$?
check "install.sh's verify() checks the recipe-run-phase ledger component" "$rc"
