...
assert len(ids) == 28, f'expected 28 catalog entries, got {len(ids)}'
"
check "generate-capability emits exactly 28 catalog entries with unique ids" "$?"
...
assert by_id['recipe-workspace']['task_id'] == 'TASK-059'
