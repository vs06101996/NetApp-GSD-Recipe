...
lines.append("recipe-onboard <new source>  (archives prior branch context; starts fresh)")
...
lines.append("- Fresh vs resume: any explicit source archives and switches out the prior active PRD/planning context before intake. `recipe-onboard` with no source resumes artifact-aware onboarding.")
lines.append("- Branch isolation: git checkout/switch saves and restores branch-local context automatically. Inspect with `recipe-workspace status`.")
