...
WORKFLOW_GROUPS = [
    ("Install and verify", [
        "recipe-start", "recipe-status", "recipe-workspace", "recipe-update",
...
lines.append("recipe-onboard <new source>  (creates clean gsd/<slug> initiative branch)")
...
lines.append("")
lines.append("`recipe-prd-intake` and `recipe-onboard` accept a Jira issue key or browse URL (fetched via Atlassian MCP), a Jira/Confluence PRD export, an already-canonical PRD, pasted text, or a freeform description.")
lines.append("")
lines.append("- Existing ticket: `recipe-onboard KAN-53` or `recipe-onboard https://example.atlassian.net/browse/KAN-53`. Intake fetches summary/description; onboard **links** that key (does not create a second Epic). `--skip-tracker` still fetches for the PRD but does not write STATE.")
lines.append("- Fresh vs resume: any explicit source creates a clean initiative branch before intake. `--branch NAME` overrides; `--no-branch` archives in place. `recipe-onboard` with no source resumes artifact-aware onboarding.")
lines.append("- Branch isolation: git checkout/switch saves and restores planning, PRDs, tracker queue/ledger, and readiness state automatically. Inspect with `recipe-workspace status`.")
lines.append("- For the official NetApp 15-section Jira/Confluence shape, start from `.templates/JIRA-PRD.input.template.md`.")
lines.append("- Mapping rules are documented in `.templates/JIRA-PRD.input.MAPPING.md`.")
lines.append("- The Jira/Confluence shape is **input only**. Intake always creates canonical `docs/PRD.md` using `.templates/PRD.template.md`.")
lines.append("- File examples: `recipe-prd-intake @path/to/jira-prd.md` or `recipe-onboard @path/to/jira-prd.md`.")
lines.append("")
