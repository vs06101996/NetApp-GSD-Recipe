...
CATALOG = [
    ...
    },
    {
        ...
        "task_id": "TASK-016",
        "kind": "cursor-skill",
        "description": "PRD intake wrapper: fills .templates/PRD.template.md from a Jira issue key/URL (Atlassian MCP getJiraIssue), Jira/Confluence export, file, paste, or freeform description; writes docs/PRD.md; invokes fotw-observer-bootstrap as its final step.",
    ...
    },
    {
        ...
        "task_id": "TASK-022",
        "kind": "cursor-skill",
        "description": "Idempotent .knowledge/ skeleton scaffold + native gsd-map-codebase, always-try gsd-graphify build (circuit breaker), and optional gsd-ingest-docs bootstrap-or-refresh wrapper.",
    ...
    },
    {
        ...
        "task_id": "TASK-026",
        "kind": "cursor-skill",
        "description": "Gate-and-invoke wrapper calling native gsd-code-review N, syncing review_complete via gsd-jira-sync (required 'In Review' transition), then calling native gsd-ship N [--draft] and surfacing the resulting PR link.",
    ...
    },
    {
        ...
        "task_id": "TASK-028",
        "kind": "cursor-skill",
        "description": "Posts GSD lifecycle milestones as Jira comments and required status transitions via Atlassian MCP, emitting matching KPI stamps; single-event mode plus Drain mode. Invoked by name by tracker-sync and every recipe-* skill that syncs a lifecycle event.",
    ...
    },
    {
        ...
        "task_id": "TASK-033",
        "kind": "cursor-skill",
        "description": "PRD -> Jira Epic bridge: drafts summary/description from docs/PRD.md, resolves project/issue type, looks up assignee (lookupJiraAccountId), creates the Epic via createJiraIssue with assignee_account_id, transitions to To Do, links STATE.md via init-tracker, and syncs intake_started via gsd-jira-sync.",
    ...
    },
    {
        ...
        "task_id": "TASK-034",
        "kind": "cursor-skill",
        "description": "Agent-mediated phase-task creation with verified Jira hierarchy: create a standard Task with native Parent when supported, explicit legacy Epic Link fallback otherwise, reject orphans, assign and transition to To Do, then mark-done/mark-failed.",
    ...
    {
        "id": "recipe-report-issue",
        "task_id": "TASK-062",
        "kind": "cursor-skill",
        "description": "Safe GitHub issue reporter for the fixed vs06101996/NetApp-GSD-Recipe repository. Drafts and redacts a structured report, previews before confirmation, checks duplicate titles, and creates bug, enhancement, or question issues through authenticated gh.",
        "invoke_name": "recipe-report-issue",
        "staged_path": ".cursor/skills/recipe-report-issue/SKILL.md",
        "installer": ".gsd-recipe/scripts/install-recipe-report-issue.sh",
        "composed_by_install_sh": True,
        "ledger_component": "recipe-report-issue",
    },
    {
    ...
    },
    {
        ...
        "task_id": "TASK-037",
        "kind": "cursor-skill",
        "description": "Single onboarding orchestrator closing the 'no single on-ramp' SDLC coverage gap. Chains, in order, whichever of recipe-prd-intake / recipe-new-project / recipe-create-epic / recipe-create-phase-tasks are actually missing their artifact (docs/PRD.md, .planning/ROADMAP.md, a linked Jira Epic, per-phase Jira sub-tasks), after one soft preview-then-confirm gate. Accepts a Jira issue key or browse URL: fetch for PRD, then link the existing ticket via init-tracker (do not create a second Epic; skip automatic phase-task creation). Once docs/PRD.md exists (intake ran or skipped), invokes fotw-observer-bootstrap; observer no-op does not fail the chain. Stops the whole chain on the first required step that fails or is declined; never re-implements any invoked skill's own logic.",
    ...
    },
    {
        ...
        "task_id": "TASK-056",
        "kind": "cursor-skill",
        "description": "First-run coach: after install, prints the next friendly Cursor command (how to onboard a PRD, then bootstrap/plan/run). Read-only unless you say Yes to invoke the recommended skill. Shares the recipe-next.sh resolver with recipe-help --next.",
        "invoke_name": "recipe-start",
        "staged_path": ".cursor/skills/recipe-start/SKILL.md",
        "installer": ".gsd-recipe/scripts/install-recipe-start.sh",
        "composed_by_install_sh": True,
        "ledger_component": "recipe-start",
    },
    {
        "id": "recipe-status",
        "task_id": "TASK-060",
        "kind": "cursor-skill",
        "description": "Read-only status snapshot: branch, PRD/ROADMAP/Epic/phase keys, PLAN/SUMMARY, install and sync hints, then the same next-step as recipe-start. Never writes files. Helper: recipe-status.sh.",
        "invoke_name": "recipe-status",
        "staged_path": ".cursor/skills/recipe-status/SKILL.md",
        "installer": ".gsd-recipe/scripts/install-recipe-status.sh",
        "composed_by_install_sh": True,
    ...
    },
    {
        ...
        "task_id": "TASK-059",
        "kind": "cursor-skill",
        "description": "Per-branch recipe workspace isolation: git post-checkout snapshots/restores .planning and untracked PRD files. Also archives and clears the active context before explicit fresh onboarding.",
        "invoke_name": "recipe-workspace",
        "staged_path": ".cursor/skills/recipe-workspace/SKILL.md",
        "installer": ".gsd-recipe/scripts/install-recipe-workspace.sh",
        "composed_by_install_sh": True,
        "ledger_component": "recipe-workspace",
    },
    {
        "id": "recipe-update",
        "task_id": "TASK-058",
    ...
    },
    {
        ...
        "task_id": "TASK-063",
        "kind": "cursor-skill",
        "description": "Manual standalone entry to the vendored Matt Pocock grilling sheet. recipe-plan-phase runs the same accuracy pass internally in Cursor Plan mode and writes .planning/GRILL.md. Does not replace recipe-prd-intake or install the full pack.",
        "invoke_name": "recipe-grill",
        "staged_path": ".cursor/skills/recipe-grill/SKILL.md",
        "installer": ".gsd-recipe/scripts/install-recipe-pocock.sh",
        "composed_by_install_sh": True,
        "ledger_component": "recipe-pocock-skills",
    },
    {
        "id": "recipe-tdd",
        "task_id": "TASK-063",
        "kind": "agent-skill",
        "description": "Injected TDD adapter for gsd-planner and gsd-executor: vendored Pocock red-green-refactor mapped onto .planning/ and docs/PRD.md. Not invoke-by-name.",
        "invoke_name": None,
        "staged_path": "skills/recipe-tdd/SKILL.md",
        "installer": ".gsd-recipe/scripts/install-recipe-pocock.sh",
        "composed_by_install_sh": True,
        "ledger_component": "recipe-pocock-skills",
    },
    {
        ...
        "task_id": "TASK-038",
        "kind": "cursor-skill",
        "description": "Read-only command reference for the NetApp GSD recipe: recipe skills, native GSD summary, harness CLI, and bench runners. Default tour is the catalog. `recipe-help --next` prints the same next-step coach as recipe-start (does not mutate the repo).",
