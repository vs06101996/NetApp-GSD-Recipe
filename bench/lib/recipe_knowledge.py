...
def assess(target: str, *, require_marker: bool = True) -> dict:
    checks: dict[str, tuple[bool, str]] = {}
    if require_marker:
        checks["marker"] = marker_ok(target)
    checks["codebase_map"] = codebase_map_ok(target)
    checks["graph_output"] = graph_output_ok(target)
...
def knowledge_ready(target: str) -> bool:
    return assess(target, require_marker=True)["ready"]


def write_marker(target: str) -> None:
    report = assess(target, require_marker=False)
    if not report["ready"]:
