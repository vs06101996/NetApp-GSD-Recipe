...
def short_sha():
    # Resolve the real commit so the displayed idempotency key matches what
    # sync-ledger.sh would actually compute if invoked with --commit "$(this value)".
    # A hardcoded "HEAD" literal here would silently diverge from the real ledger key.
    try:
        return subprocess.check_output(
            ["git", "-C", repo_root, "rev-parse", "--short", "HEAD"], text=True, stderr=subprocess.DEVNULL
        ).strip()
    except Exception:
        return "HEAD"
