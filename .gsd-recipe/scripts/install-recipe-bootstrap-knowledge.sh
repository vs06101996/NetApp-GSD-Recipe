#!/usr/bin/env bash
# recipe-bootstrap-knowledge installer — standalone fallback path per
...
# composed into it in a follow-up integration pass — see
# bench/report/recipe-bootstrap-knowledge-integration-report.md for the exact
# RECIPE_BOOTSTRAP_KNOWLEDGE_INSTALLER wiring snippet, held back from this
# task's own diff to avoid a merge conflict with sibling tasks concurrently
# editing install.sh).
#
...
#
# Usage:
#   ./.gsd-recipe/scripts/install-recipe-bootstrap-knowledge.sh [--yes] [--target <repo_root>]
#   ./.gsd-recipe/scripts/install-recipe-bootstrap-knowledge.sh --uninstall [--target <repo_root>]
#
...
#     under component "recipe-bootstrap-knowledge" for clean removal.
...
#     .knowledge/ or code_base_details/ itself — that scaffolding is the
#     staged skill's own runtime job (see SKILL.md § C.1), not the
#     installer's.
set -euo pipefail
...
if [ ! -d "$TARGET/.git" ]; then
  echo "recipe-bootstrap-knowledge installer: $TARGET is not a git repo root. Refusing to scaffold (fail closed)." >&2
  exit 1
fi

GSD_RECIPE_DIR="$TARGET/.gsd-recipe"
LEDGER="$GSD_RECIPE_DIR/ledger.json"
SKILL_DEST="$TARGET/.cursor/skills/recipe-bootstrap-knowledge/SKILL.md"
SKILL_SRC="$SCRIPT_DIR/../templates/recipe-bootstrap-knowledge-SKILL.md"
COMPONENT="recipe-bootstrap-knowledge"
...
install() {
  if [ "$YES" -ne 1 ]; then
    read -r -p "Install recipe-bootstrap-knowledge skill (standalone, ahead of TASK-010 composition) into $TARGET? [y/N] " reply
    case "$reply" in
      [yY]|[yY][eE][sS]) : ;;
      *) echo "recipe-bootstrap-knowledge installer: aborted, no consent given."; exit 0 ;;
    esac
  fi

  safe_copy "$SKILL_SRC" "$SKILL_DEST"
  ledger_record ".cursor/skills/recipe-bootstrap-knowledge/SKILL.md"

  echo "recipe-bootstrap-knowledge installer: staged. Files tracked in $LEDGER:"
  ledger_files | sed 's/^/  - /'
  echo
...
uninstall() {
  echo "recipe-bootstrap-knowledge installer: removing tracked files for component '$COMPONENT'..."
  ...
  rmdir "$TARGET/.cursor/skills/recipe-bootstrap-knowledge" 2>/dev/null || true
...
with open(ledger_path, "w") as f:
    ...
    f.write("\n")
PY
  echo "recipe-bootstrap-knowledge installer: uninstall complete. .knowledge/ and code_base_details/ (if the skill ran) are left in place — never owned/removed by this installer."
}
