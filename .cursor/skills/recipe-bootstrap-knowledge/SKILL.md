...
- Do not write `.gsd-recipe/KNOWLEDGE-BOOTSTRAPPED` with the Write tool — only
  `recipe-verify-knowledge.sh --write-marker` may create or refresh it.
- Do not hand-write `.planning/codebase/*.md` or fabricate graph files — native commands must produce them.
