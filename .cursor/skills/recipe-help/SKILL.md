...
| `--stuck` | Short FAQ: clone lost skills, PRD input formats, no Epic, unassigned tickets, status did not move |
...
2. **Apply the requested mode** from § A:
   ...
   - **`--stuck`:** Print this FAQ (do not invent product-specific names):
     ...
     4. Starting different work while an old ROADMAP is visible — pass the new source to `recipe-onboard`. It creates a clean `feat/<title>[-<Ticket>]` (or `fix/...`) initiative branch before intake (`--no-branch` archives in place). Branch changes swap planning and tracker state automatically; inspect with `recipe-workspace status`.
...
→ recipe-onboard  (includes mandatory knowledge bootstrap)
