...
| OD-23 | Initiative branch is the onboarding boundary | Fresh `recipe-onboard <source>` creates a new `gsd/<slug>` branch **before intake**. This turns TASK-059's branch-local snapshots into initiative isolation even when onboarding starts on shared `main`/`master`. `--branch NAME` overrides the derived name; `--no-branch` explicitly chooses TASK-054 archive-in-place. Fail closed on dirty product files, detached HEAD, collisions, or missing runtime; never auto-stash, reuse a branch, or invent suffixes. Branch creation and archive-in-place are mutually exclusive. See [BACKLOG TASK-061](BACKLOG.md). | Implemented (TASK-061) |
