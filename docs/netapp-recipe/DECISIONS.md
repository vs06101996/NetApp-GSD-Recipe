...
| OD-23 | Initiative branch is the onboarding boundary | Fresh `recipe-onboard <source>` creates a new `gsd/<slug>` branch **before intake**. From `main`/`master` it uses the current branch; from another initiative it bases the new branch on `origin/HEAD`, then local `main`/`master`, preventing PR 2 from containing PR 1. This turns TASK-059's snapshots into initiative isolation. `--branch NAME` overrides; `--no-branch` chooses TASK-054 archive-in-place. Fail closed on dirty/tracked state, detached HEAD, collisions, or missing runtime; never auto-stash, reuse a branch, or invent suffixes. See [BACKLOG TASK-061](BACKLOG.md). | Implemented (TASK-061) |
