...
| `recipe-onboard` skill (TASK-037) | **Built** — single onboarding orchestrator closing the "no single on-ramp" gap: an invoke-by-name skill that determines, read-only, which of `docs/PRD.md`/`.planning/ROADMAP.md`/a linked Jira Epic/per-phase Jira sub-tasks already exist, shows one soft preview-then-confirm gate for the whole chain, then invokes whichever of `recipe-prd-intake` → `recipe-new-project` → `recipe-create-epic` → `recipe-create-phase-tasks` are actually missing their artifact, skipping the rest. Stops the whole chain immediately on the first step that fails/declines; never re-implements any invoked skill's own logic or duplicates its Jira sync. Falls back to native `gsd-new-project` for the project-bootstrap step only when `recipe-new-project` (TASK-036, not yet built) isn't staged. `.gsd-recipe/scripts/install-recipe-onboard.sh` (standalone installer composed into `install.sh` as a sub-installer) — see `.gsd-recipe/templates/recipe-onboard-SKILL.md` |
...
| `recipe-install [--target <path>] [--uninstall]` (TASK-031) | Thin end-to-end install orchestrator chaining `recipe-validate-tokens` → a live human consent gate → `install.sh --yes` → `recipe-install-verify` → one combined summary; `--uninstall` delegates to `install.sh --uninstall --yes` behind its own live confirm gate — [.gsd-recipe/templates/recipe-install-SKILL.md](../../.gsd-recipe/templates/recipe-install-SKILL.md) · [report](../../bench/report/recipe-install-integration-report.md) |
...
| `recipe-onboard [<PRD source>] [--project KEY] [--issue-type NAME] [--force]` (TASK-037) | Single onboarding orchestrator: one preview-then-confirm gate, then chains whichever of `recipe-prd-intake` → `recipe-new-project` → `recipe-create-epic` → `recipe-create-phase-tasks` is missing its artifact, skipping the rest — stops on first failure/decline — [.gsd-recipe/templates/recipe-onboard-SKILL.md](../../.gsd-recipe/templates/recipe-onboard-SKILL.md) |
...
| Doc | Purpose |
|-----|---------|
| [DEMO.md](DEMO.md) | Quick demo — Jira-only path, 5 commands, ~10 min |
...
| [BACKLOG.md](BACKLOG.md) | Implementation tasks (start Wave 1) |
