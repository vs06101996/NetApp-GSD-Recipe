...
| `recipe-report-issue` skill (TASK-062) | **Built** — drafts and redacts a structured bug, enhancement, or question for the fixed `vs06101996/NetApp-GSD-Recipe` repository; dry-run and preview precede a non-skippable confirmation, duplicate titles are checked, and authenticated `gh issue create` returns the created URL. No project files or logs are attached automatically. |
...
| `recipe-onboard` skill (TASK-037/054/061) | **Built** — no source resumes artifact-aware onboarding; an explicit Jira/file/paste/description source starts fresh on a new `feat/<feature-title>[-<Ticket>]` (or `fix/...` with `--fix`) initiative branch by default. It snapshots prior planning, PRDs, tracker queue/ledger, and readiness state through TASK-059 before intake, so development contexts cannot leak through shared `main`/`master`. `--branch NAME` overrides; `--no-branch` archives in place. Jira issue input links the existing issue instead of creating a duplicate Epic. |
...
| Pocock adapters (TASK-063) | **Built** — `recipe-plan-phase` runs vendored grilling internally in Cursor Plan mode; `recipe-grill` remains a manual entry point. Includes injected `recipe-tdd` and two-axis append on `recipe-review-ship`. Not a second SDLC. |
| Cursor install plugin (TASK-064) | **Built** — [`extensions/netapp-gsd-recipe`](../../extensions/netapp-gsd-recipe/README.md) wraps bash install and prompt prefills; gates unchanged ([OD-24](DECISIONS.md)). |
...
| PO + CI green for phase N | `settled` | Phase task; aggregate status-only roll-up may close Epic |
